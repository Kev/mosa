...
void printOptions(char* name)
{
        ...
        fprintf(stderr,"        -x      --              Size of the state sets\n");
        fprintf(stderr,"        -g      --              Time to greedy\n");
				fprintf(stderr,"				-G			--							Enable pqrs griddy thingy\n");
...
int main(int argc, char *argv[])
{
  ...
  //First off, lets parse the command-line arguments
  char* short_options="d:i:p:s:x:g:GOVvh";
  int option_char;
  while ((option_char = getopt (argc, argv, short_options)) != -1)
  {
    switch (option_char)
      ...
      case 'x': initialSetSize = atoi(optarg);break;
      case 'g': lowTemperatureAfter = atoi(optarg);break;
			case 'G': pqrs = true;break;
  ...
  //lets test everything's set correctly
  ASSERT( numObjectives > 0);
  ASSERT( numParameters > 0);

  DominanceAnnealer *sa = new DominanceAnnealer(numParameters, numObjectives, initialSetSize, problem, startingTemperature, coolingFactor, coolingFrequency, pqrs);
